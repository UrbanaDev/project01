	...
	$.fn.jCounter = function(options,callback) {
		...
		function countdown_proc(duration,el) {
			...
			if(!isNaN(eventDate)) {
				...
				$(el).find(".day_bar").css('height',(days*(100/365))+"%");
				$(el).find(".hour_bar").css('height',(hours*(100/24))+"%");
				$(el).find(".min_bar").css('height',(minutes*(100/60))+"%");
				$(el).find(".sec_bar").css('height',(seconds*(100/60))+"%");
